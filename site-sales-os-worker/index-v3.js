...
async function geocodeCity(city) {
  ...
  const r = await fetch(url, { headers: { 'User-Agent': 'site-sales-os/0.3', 'Accept-Language': 'pt-BR,pt;q=0.9' } });
...
export default {
  async fetch(request, env) {
    ...
    if (p === '/api/health') return json({ ok: true, service: 'site-sales-os', version: 3 });
